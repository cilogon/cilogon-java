...
cd $CILOGON_QDL_DEPLOY || exit
java  org.qdl_lang.install.ListDistroFiles $CILOGON_QDL_DEPLOY
# Get the actual manifest so that build info is available.
jar cmf installer.mf "$JAR_NAME" org/qdl_lang/install/Installer.class dir_list.txt file_list.txt version.txt  bin docs etc lib log var examples
